...
import { CanvasObjectData } from '@/types/liveblocks';
...
type ConnectionState = 'connecting' | 'connected' | 'disconnected' | 'initial' | 'reconnecting';

interface WhiteboardProps {
    initialData?: object | null;
}

export const Whiteboard = forwardRef<fabric.Canvas, WhiteboardProps>(({ initialData }, ref) => {
    ...
    //         });
    //     }
    // }, [connectionState, updateMyPresence, activeTool, self?.info?.name]);
    ...
    const updateObjectInStorage = useMutation(
        (
            mutation,
            objectId: string,
            updateData: Partial<CanvasObjectData>
        ) => {
            ...
            function isLiveObject(value: any): value is LiveObject<any> {
            ...
            try {
                ...
                const newData = isLiveObject(data) ? { ...data.toObject() } : {};
                ...
                if (isLiveObject(data)) {
                    data.update(newData); // ✅ Type-safe
                } else {
                    console.warn("data is not a LiveObject or is undefined");
                }
    ...
    const throttledUpdateObject = useMemo(
        () => throttle(
            (objectId: string, updateData: Partial<CanvasObjectData>) => {
                if (objectId) {
    ...
    }, [room, updateMyPresence, activeTool]);
    ...
    useEffect(() => {
        ...
        switch (activeTool) {
            case 'pen':
                ...
                canvas.forEachObject((obj: fabric.Object) => obj.set({ evented: false, selectable: false }));
                break;
    ...
    useEffect(() => {
        ...
        if (currentCanvasObjects && ((currentCanvasObjects as unknown) as LiveList<CanvasObject>).length > 0) {
            ...
            function getCanvasObjectData(obj: CanvasObject | any) {
                if (typeof obj.get === 'function') {
                    return { type: obj.get('type'), data: obj.get('data') };
                } else {
                    return { type: obj.type, data: obj.data };
                }
            }

            const objectsToProcess = ((currentCanvasObjects as unknown) as LiveList<CanvasObject>).map(getCanvasObjectData);


            // Extract just the data which contains the serialized Fabric props
            const dataToEnliven = objectsToProcess.map((p: { data: any }) => p.data);
    ...
    const handleDeleteKeyDown = useCallback((e: KeyboardEvent) => {
        if ((e.key === 'Delete' || e.key === 'Backspace') && activeTool === 'select') {
            ...
            objectsToDelete.forEach((obj: fabric.Object) => {
    ...
    const handleObjectMoving = useCallback((options: FabricEventWithTarget) => {
        ...
        if (objectId) {
            ...
            const updateData: Partial<CanvasObjectData> = { left, top };
    ...
    const handleObjectScaling = useCallback((options: FabricEventWithTarget) => {
        ...
        if (objectId) {
            const updateData: Partial<CanvasObjectData> = {
                left: target.left,
                top: target.top,
    ...
    const handleObjectModified = useCallback((options: fabric.ModifiedEvent<TPointerEvent>) => {
        ...
        if (modifiedObject.type === 'i-text') {
            const textValue = (modifiedObject as fabric.IText).text;
            if (textValue !== undefined) {
                updateObjectInStorage(objectId, { text: textValue });
            }
            return;
        }

        if (activeTool === 'select') {
            const left = typeof modifiedObject.left === 'number' ? modifiedObject.left : 0;
            const top = typeof modifiedObject.top === 'number' ? modifiedObject.top : 0;
            const scaleX = typeof modifiedObject.scaleX === 'number' ? modifiedObject.scaleX : 1;
            const scaleY = typeof modifiedObject.scaleY === 'number' ? modifiedObject.scaleY : 1;
            const angle = typeof modifiedObject.angle === 'number' ? modifiedObject.angle : 0;

            const finalProps: Partial<CanvasObjectData> = {
                left,
                top,
                scaleX,
                scaleY,
            ...
            updateObjectInStorage(objectId, finalProps);
        }
    }, [activeTool, updateObjectInStorage]);

    // useEffect(() => {
    //     // should re-run after updateObjectInStorage is called
    //     console.log("🔁 canvasObjects updated");
    // }, [canvasObjects]);
    ...
    const [, setUndoStack] = useState<UndoableAction[]>([]);
    // Initialize the redo stack as an empty array of UndoableAction
    const [, setRedoStack] = useState<UndoableAction[]>([]);
    ...
    const handleMouseDown = useCallback((options: FabricEventWithTarget) => {
        ...
        if (currentTool === 'rectangle' || currentTool === 'circle') {
            ...
            const shape: fabric.Object = currentTool === 'rectangle'
                ? new fabric.Rect({
                    left: pointer.x,
                    top: pointer.y,
                    width: 1,
        ...
        if (currentTool === 'text') {
            ...
            const propertiesToInclude: (keyof fabric.ITextOptions)[] = [
                'id',
                'type',
                'text',
                'left',
